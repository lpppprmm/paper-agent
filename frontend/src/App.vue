  ...
  <main class="app-shell" :class="{ expanded: isExpanded }">
    ...
    <div v-else class="layout layout-fullscreen">
      ...
      >
        ...
        >
          <h3>{{ form.mode === "paper" ? "学术论文" : "最终报告" }}</h3>
  ...
  try {
    await runResearchStream(
      ...
      (event: ResearchStreamEvent) => {
        ...
        if (event.type === "argument") {
          const payload = event as Record<string, unknown>;
          const args = Array.isArray(payload.arguments)
            ? payload.arguments
            : [];
          progressLogs.value.push(`已构建论点框架（${args.length} 个论点）`);
          if (payload.fallback === true) {
            progressLogs.value.push("论点构建降级为兜底模式");
          }
          return;
        }
        ...
        if (event.type === "outline") {
          ...
          const sections = Array.isArray(payload.sections)
            ? payload.sections
            : [];
          progressLogs.value.push(`已生成论文大纲（${sections.length} 节）`);
          return;
        }

        if (event.type === "reference_pool") {
          const payload = event as Record<string, unknown>;
          const sources = Array.isArray(payload.sources)
            ? payload.sources
            : [];
          progressLogs.value.push(`引用池已建立（${sources.length} 条来源）`);
          return;
        }

        if (event.type === "paper_section_start") {
          const title =
            typeof event.title === "string" && event.title.trim()
              ? event.title.trim()
              : "未命名章节";
          progressLogs.value.push(`开始撰写章节：${title}`);
          return;
        }

        if (event.type === "paper_section_chunk") {
          const chunk =
            typeof event.content === "string" ? event.content : "";
        ...
        if (event.type === "paper_section_end") {
          const title =
            typeof event.title === "string" && event.title.trim()
              ? event.title.trim()
              : "未命名章节";
          progressLogs.value.push(`章节完成：${title}`);
          return;
        }

        if (event.type === "paper_abstract") {
          progressLogs.value.push("摘要与关键词已生成");
          return;
        }

        if (event.type === "more_research_start") {
          const payload = event as Record<string, unknown>;
          const deficient = Array.isArray(payload.deficient)
            ? payload.deficient
            : [];
          progressLogs.value.push(
            `检测到 ${deficient.length} 个论点证据不足，启动补充检索`
          );
          return;
        }

        if (event.type === "more_research_sources") {
          const payload = event as Record<string, unknown>;
          const refs = Array.isArray(payload.source_refs)
            ? payload.source_refs
            : [];
          progressLogs.value.push(
            `论点 ${payload.argument_id} 补充了 ${refs.length} 条来源`
          );
          return;
        }

        if (event.type === "more_research_done") {
          const payload = event as Record<string, unknown>;
          progressLogs.value.push(
            `补充检索完成，新增 ${payload.total_new_sources} 条来源`
          );
          return;
        }

        if (event.type === "final_paper") {
          const paper =
            typeof event.paper === "string" && event.paper.trim()
              ? event.paper.trim()
              : "";
          reportMarkdown.value = paper || "论文生成失败，未获得有效内容";
          pulse(reportHighlight);
          progressLogs.value.push("学术论文已生成");
          return;
        }
